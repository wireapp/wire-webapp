...
export class Backend {
  ...
  public async createPersonalUser(user: User) {
    // 1. Register
    ...
    const activationCode = await this.getActivationCodeForEmail(user.email);

    // 3. Activate Account
    await this.activateAccount(user.email, activationCode);

    // 4. Request Access Token
    user.token = await this.requestAccessToken(zuidCookie);

    // 5. Set Unique Username (Handle)
    await this.setUniqueUsername(user.username, user.token);
  }
  ...
  private async registerUser(user: User): Promise<AxiosResponse> {
    return await this.axiosInstance.post('register', {
  ...
  private async getActivationCodeForEmail(email: string): Promise<string> {
  ...
  private async activateAccount(email: string, code: string) {
  ...
  private async requestAccessToken(zuidCookie: string): Promise<string> {
  ...
  private async setUniqueUsername(username: string, token: string) {
