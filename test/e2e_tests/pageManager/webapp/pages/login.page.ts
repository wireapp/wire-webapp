...
export class LoginPage {
  ...
  async login(user: Pick<User, 'email' | 'password'>) {
     ...
     * Using 'load' instead of 'networkidle' to handle cases where background requests continue after login
     */
    await this.page.waitForURL(new RegExp(`^${webAppPath}$`), {timeout: 40_000, waitUntil: 'load'});
  }
}
