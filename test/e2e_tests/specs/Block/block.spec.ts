...
test.describe('User Blocking', () => {
  test.describe('Block: User A and User B are NOT in the same team', () => {
    ...
    test.beforeEach(async ({createTeam}) => {
      const teamA = await createTeam('Team A', {withMembers: 1});
    ...
    test(
      'I want to cancel blocking a 1:1 conversation from conversation list 0',
    ...
    test(
      ...
      async ({createPage}) => {
        const userAContext = await PageManager.from(createPage(withLogin(userA), withConnectionRequest(userB)));
        const userAPageManager = userAContext.webapp;
        ...
        await test.step('Step 2: User A blocks User B from group conversation', async () => {
          ...
          await blockUserFromOpenGroupProfileView(userAContext, userB);
        });
    ...
    test(
      ...
      async ({createPage}) => {
        ...
        await blockUserFromProfileView(userAContext, userB);
    ...
    test(
      ...
      async ({createPage}) => {
        const userAContext = await PageManager.from(
        ...
        // Preconditions: User B accepts the connection request
        const userBContext = await PageManager.from(createPage(withLogin(userB)));
        const userBPages = userBContext.webapp.pages;
        await userBPages.connectRequest().clickConnectButton();
        ...
        await test.step('User B blocks User A', async () => {
          ...
          await blockUserFromConversationList(userBContext, userA);
        });
    ...
    test(
      ...
      async ({createPage}) => {
        ...
        await blockUserFromConversationList(userAContext, userB);
        // Step 3: User B does not see Connection Request from User A
        await expect(userBPages.conversationList().getConversationLocator(userA.fullName)).not.toBeVisible();
      },
    );
  ...
  test.describe('Block: User A and User B are in the same team', () => {
    ...
    test('Verify you can not block a user from your team', {tag: ['@TC-8778', '@regression']}, async ({createPage}) => {
      const userAContext = await PageManager.from(createPage(withLogin(userA), withConnectedUser(userB)));
      const userAPageManager = userAContext.webapp;
      const userAPages = userAPageManager.pages;
