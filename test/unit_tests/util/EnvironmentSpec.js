...
describe('EnvironmentSpec', () => {
  describe('z.util.Environment._electronVersion', () => {
    it('detects wrapper version for internal', () => {
      ...
      expect(z.util.Environment._electronVersion(userAgent)).toBe('2.14.2744');
    });
    ...
    it('detects wrapper version for public', () => {
      ...
      expect(z.util.Environment._electronVersion(userAgent)).toBe('2.13.2734');
    });
    ...
    it('return undefined if no version is present', () => {
      ...
      expect(z.util.Environment._electronVersion(userAgent)).not.toBeDefined();
    });
  });
});
