...
describe 'z.conversation.ConversationRepository', ->
  ...
  describe 'merge_conversations', ->
    it 'accumulates local data with remote data from the backend', ->
      ...
      merged_conversations = conversation_repository.merge_conversations [local_data], [remote_data]
    ...
    it 'only maps other participants if they are still in the conversation', ->
      ...
      merged_conversations = conversation_repository.merge_conversations [], [remote_data]
      merged_data = merged_conversations[0]
