...
describe('Event Mapper', () => {
  ...
  describe('mapJsonEvent', () => {
    ...
    it('skips messages which cannot be mapped', () => {
      // prettier-ignore
      /* eslint-disable comma-spacing, key-spacing, sort-keys, quotes */
      const goodMessage = {"conversation":conversationEt.id,"id":"4cec0f75-d963-486d-9401-415240ac2ad8","from":"532af01e-1e24-4366-aacf-33b67d4ee376","time":"2016-08-04T15:12:12.453Z","data":{"content":"Message with timestamp","nonce":"4cec0f75-d963-486d-9401-415240ac2ad8","previews":[]},"type":"conversation.message-add"};
      // prettier-ignore
      const badMessage = {"conversation":conversationEt.id,"id":"aeac8355-739b-4dfc-a119-891a52c6a8dc","from":"532af01e-1e24-4366-aacf-33b67d4ee376","data":{"content":"Knock, are you there? :)","nonce":"aeac8355-739b-4dfc-a119-891a52c6a8dc"},"type":"conversation.message-add"};
      /* eslint-enable comma-spacing, key-spacing, sort-keys, quotes */

      return eventMapper.mapJsonEvents([goodMessage, badMessage], conversationEt).then(messageEntities => {
    ...
    it('filters mentions that are out of range', () => {
      ...
      eventMapper.mapJsonEvent(event, conversationEntity).then(messageEntity => {
  ...
  describe('_mapAssetImage', () => {
    it('detects a malformed asset key', () => {
      ...
      const assetEt = eventMapper._mapAssetImage(event, true);

      try {
        assetEt.resource().generateUrl();
      } catch (error) {
  ...
  describe('_mapEventUnableToDecrypt', () => {
    it('maps a message from a decrypt error event', () => {
      ...
      const messageEt = eventMapper._mapEventUnableToDecrypt(event);

      expect(messageEt.error_code).toBe('205');
      expect(messageEt.client_id).toBe('c0a70d96aaeb87b6');
    });
  });
});
