...
window.TestFactory = class TestFactory {
  ...
  async exposeAuthActors() {
    TestFactory.auth_repository = new AuthRepository(new AuthService(backendClient));
  ...
  async exposeCryptographyActors(mockCryptobox = true) {
    ...
    TestFactory.cryptography_service = new CryptographyService(backendClient);

    TestFactory.cryptography_repository = new CryptographyRepository(backendClient, TestFactory.storage_repository);
  ...
  async exposeEventActors() {
    ...
    TestFactory.web_socket_service = new WebSocketService(backendClient, TestFactory.storage_service);
    ...
    TestFactory.notification_service = new NotificationService(backendClient, TestFactory.storage_service);
  ...
  async exposeUserActors() {
    ...
    TestFactory.asset_service = new AssetService(backendClient);
    TestFactory.connection_service = new ConnectionService(backendClient);
    TestFactory.user_service = new UserService(backendClient, TestFactory.storage_service);
  ...
  async exposeConnectionActors() {
    ...
    TestFactory.connection_repository = new ConnectionRepository(backendClient, TestFactory.user_repository);
  ...
  async exposeSearchActors() {
    await this.exposeUserActors();
    TestFactory.search_repository = new SearchRepository(backendClient, TestFactory.user_repository);
  ...
  async exposeTeamActors() {
    await this.exposeUserActors();
    TestFactory.team_repository = new TeamRepository(backendClient, TestFactory.user_repository);
  ...
  async exposeConversationActors() {
    ...
    TestFactory.conversation_repository = new ConversationRepository(
      ...
      undefined,
      new LinkPreviewRepository(new AssetService(backendClient), propertiesRepository),
      new MessageSender(),
      serverTimeHandler,
