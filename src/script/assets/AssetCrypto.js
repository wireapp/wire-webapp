...
class AssetCrypto {
   ...
   */
  static decryptAesAsset(cipherText, keyBytes, referenceSha256) {
    return window.crypto.subtle
      .digest('SHA-256', cipherText)
      .then(computedSha256 => {
        if (AssetCrypto._equalHashes(referenceSha256, computedSha256)) {
   ...
   */
  static encryptAesAsset(plaintext) {
    const iv = AssetCrypto._generateRandomBytes(16);
    const rawKeyBytes = AssetCrypto._generateRandomBytes(32);
    let key = null;
  ...
  static _equalHashes(bufferA, bufferB) {
