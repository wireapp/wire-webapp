 ...
 *
 * Implements separable Gaussian blur in the vertical direction. Used as the
 * second pass of a two-pass separable blur (horizontal + vertical) for efficiency.
 *
 * Algorithm:
 * - Samples neighboring pixels in vertical direction
 ...
 *
 * Uniforms:
 *   uSrc: Source texture to blur (typically output from horizontal blur pass)
...
void main() {
  ...
  // Sample neighboring pixels in vertical direction
  for (int i = -MAX_RADIUS; i <= MAX_RADIUS; i++) {
    ...
    sum += texture(uSrc, vTexCoord + vec2(0.0, fi) * uTexelSize) * weight;
    weightSum += weight;
  }
