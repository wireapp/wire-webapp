...
const MainContent: React.FC<LeftSidebarProps> = ({
  ...
  if (!contentStateToRender) {
    return null;
  }

  return (
    <>
      <h1 className="visually-hidden">{stateTitles[state]}</h1>
          ...
          <Animated key={state}>
            <>
              {state === ContentState.COLLECTION && activeConversation && (
                <Collection conversation={activeConversation} conversationRepository={repositories.conversation} />
              )}
