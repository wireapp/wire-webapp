...
import {hasConfigDefaultProtocol, hasConfigVerificationExpiration} from '../../guards';
...
export const handleE2EIdentityFeatureChange = (logger: Logger, config: FeatureList) => {
  ...
  if (!hasConfigVerificationExpiration(e2eiConfig) && !hasConfigDefaultProtocol(mlsConfig)) {
    return;
  }
