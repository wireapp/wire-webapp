...
describe('renderMessage', () => {
  it('renders a normal link', () => {
    ...
    expect(renderMessage('Check this: http://www.wire.com/', null)).toBe(expected);
  });
  ...
  it('renders a normal link without protocol', () => {
    ...
    expect(renderMessage('Check this: wire.com/about/', null)).toBe(expected);
  });
  ...
  it('renders complicated image links', () => {
    ...
    expect(renderMessage(link, null)).toBe(expected);
  });
  ...
  it('renders URLs with underscores', () => {
    ...
    expect(renderMessage(`Stormtroopers: ${link} !!!`, null)).toBe(expected);
  });
  ...
  it('renders URLs without a trailing slash', () => {
    ...
    expect(renderMessage(`e.g. ${link}.`, null)).toBe(expected);
  });
  ...
  it('renders URLs with @-signs and text correctly', () => {
    ...
    expect(renderMessage(`Just click ${link} and download it`, null)).toBe(expected);
  });

  it('escapes links when they are posted as plain HTML', () => {
    const expected = "&lt;a href=&quot;javascript:alert('ohoh!')&quot;&gt;what?&lt;/a&gt;";

    expect(renderMessage('<a href="javascript:alert(\'ohoh!\')">what?</a>', null)).toBe(expected);
  });
  ...
  it('renders an email address', () => {
    ...
    expect(renderMessage('send it over to hello@wire.com', null)).toBe(expected);
  });
  ...
  it('renders an email address with pluses', () => {
    ...
    expect(renderMessage('send it over to hello+world@wire.com', null)).toBe(expected);
  });
  ...
  it('renders an email long domains', () => {
    ...
    expect(renderMessage('send it over to janedoe@school.university.edu', null)).toBe(expected);
  });
  ...
  it('renders an emoticon of someone shrugging', () => {
    ...
    expect(renderMessage('¯_(ツ)_/¯', null)).toBe('¯_(ツ)_/¯');
  });
  /* eslint-enable no-useless-escape */

  it('renders a link from markdown notation', () => {
    expect(renderMessage('[doop](http://www.example.com)', null)).toBe(
  ...
  it('does not render a js link from markdown notation', () => {
    expect(renderMessage("[doop](javascript:alert('nope'))", null)).toBe("[doop](javascript:alert('nope'))");
  });

  it('does not render markdown links with empty or only whitespace values', () => {
    expect(renderMessage('[](http://example.com)', null)).toBe('[](http://example.com)');
    expect(renderMessage('[   ](http://example.com)', null)).toBe('[   ](http://example.com)');
    expect(renderMessage('[link]()', null)).toBe('[link]()');
    expect(renderMessage('[link](    )', null)).toBe('[link]()');
  });

  it('renders special escaped email links from markdown notation', () => {
    expect(renderMessage('[email](mailto:test@email.com)', null)).toBe(
      '<a href="mailto:test@email.com" data-email-link="true" data-md-link="true" data-uie-name="markdown-link">email</a>',
    );

    expect(renderMessage("[email](mailto:'\\);alert\\('pwned'\\)//)", null)).toBe(
  ...
  it('renders a link from markdown notation with a title', () => {
    expect(renderMessage('[sometext](https://some.domain "this is a title")', null)).toBe(
  ...
  it('escapes links with an xss attempt on their url', () => {
    expect(renderMessage(`[sometext](https://some.domain"><script>alert("oops")</script>)`, null)).toBe(
  ...
  it('does not add a < behind URLs within <code> tags 2', () => {
    expect(renderMessage('` http://wire.com`\n`123`', null)).toBe('<code> http://wire.com</code><br><code>123</code>');
  });
  it('does not add a < behind URLs within <code> tags', () => {
    expect(renderMessage('` http://wire.com`', null)).toBe('<code> http://wire.com</code>');
  });

  it('escapes url params', () => {
    expect(renderMessage(`[sometext](https://some.domain?param1=a&param2=b)`, null)).toBe(
  ...
  it('escapes links with html charachters on their title', () => {
    expect(renderMessage(`[sometext](https://some.domain "<this> &' >that<")`, null)).toBe(
  ...
  it('conversts unicode links to punycode', () => {
    ...
    expect(renderMessage('[https://müller.de](https://müller.de)', null)).toBe(
  ...
  describe('Mentions', () => {
    ...
    tests.forEach(({expected, mentions, testCase, text}) => {
      const mentionEntities = mentions.map(mention => {
        const mentionEntity = new MentionEntity(mention.startIndex, mention.length, mention.userId, null);
...
describe('Markdown for bold text', () => {
  it('renders bold text', () => {
    expect(renderMessage('**bold text (not italic)**', null)).toBe('<strong>bold text (not italic)</strong>');
  });

  it('renders a bold word within a sentence', () => {
    expect(renderMessage('Markdown **just** rocks!', null)).toEqual('Markdown <strong>just</strong> rocks!');
  });

  it('renders bold text with italic words', () => {
    expect(renderMessage('**bold text with *italic* !!**', null)).toBe(
      '<strong>bold text with <em>italic</em> !!</strong>',
    );
  });

  it('renders text which is partly bold and partly italic', () => {
    expect(renderMessage('**part bold,** *part italic*', null)).toBe(
      '<strong>part bold,</strong> <em>part italic</em>',
    );
  });

  it('renders mixed text with bold and italic words', () => {
    expect(renderMessage('*italic* **bold** *italic* **bold**', null)).toBe(
      '<em>italic</em> <strong>bold</strong> <em>italic</em> <strong>bold</strong>',
    );
  });

  it('renders words which are just bold', () => {
    expect(renderMessage('**A**', null)).toBe('<strong>A</strong>');
  });
});

describe('Markdown for italic text', () => {
  it('renders italic text', () => {
    expect(renderMessage('*This text is italic.*', null)).toBe('<em>This text is italic.</em>');
  });

  it('renders partially italic text', () => {
    expect(renderMessage('This text is *partially* italic', null)).toBe('This text is <em>partially</em> italic');
  });

  it('renders text with multiple italic words', () => {
    expect(renderMessage('This text has *two* *italic* bits', null)).toBe(
      'This text has <em>two</em> <em>italic</em> bits',
    );
  });

  it('renders italic text with bold words', () => {
    expect(renderMessage('*italic text **with bold***', null)).toBe('<em>italic text <strong>with bold</strong></em>');
  });

  it('renders words which are bold-italic', () => {
    expect(renderMessage('***A***', null)).toBe('<em><strong>A</strong></em>');
  });
});

describe('Markdown for code snippets', () => {
  it('renders code blocks', () => {
    expect(renderMessage("```console.log('A')```", null)).toEqual("<code>console.log('A')</code>");
  });

  it('can escape HTML in rendered code blocks', () => {
    expect(renderMessage('```<b>Hello</b>```', null)).toEqual('<code>&lt;b&gt;Hello&lt;/b&gt;</code>');
  });

  it('renders code within code spans', () => {
    expect(renderMessage('This is `code`.', null)).toEqual('This is <code>code</code>.');
  });
  ...
  it(`doesn't render code within a code span`, () => {
    expect(renderMessage('`com.ibm.icu`', null)).toEqual('<code>com.ibm.icu</code>');
  });
  ...
  it('renders escaped HTML code spans', () => {
    ...
    expect(renderMessage('`<a href="javascript:wire.app.logout()">This is a trick</a>`', null)).toEqual(expected);
  });
});

describe('Markdown for headings', () => {
  it('renders all headings the same', () => {
    expect(renderMessage('# heading', null)).toBe('<div class="md-heading">heading</div>');
    expect(renderMessage('## heading', null)).toBe('<div class="md-heading">heading</div>');
    expect(renderMessage('### heading', null)).toBe('<div class="md-heading">heading</div>');
    expect(renderMessage('#### heading', null)).toBe('<div class="md-heading">heading</div>');
  });
...
describe('Ignored Markdown syntax', () => {
  it('only renders correct Markdown syntax', () => {
    expect(renderMessage('This text is not italic.', null)).toBe('This text is not italic.');
  });

  it('does not render bold text when there is only a single asterisk', () => {
    expect(renderMessage('random *asterisk', null)).toBe('random *asterisk');
  });

  it('does not render horizontal lines', () => {
    expect(renderMessage('***\nNo horizontal lines\n***', null)).toBe('***<br>No horizontal lines<br>***');
  });
  ...
  it('does not render blockquotes', () => {
    expect(renderMessage('>no blockquote', null)).toBe('&gt;no blockquote');
    expect(renderMessage('> no blockquote', null)).toBe('&gt; no blockquote');
  });
  ...
  it('does not render tables', () => {
    ...
    expect(renderMessage(input, null)).toBe(expected);
  });
});
...
describe('Markdown exceptions', () => {
  it('handles the URLs that start with : after the protocol', () => {
    ...
    expect(renderMessage(text, null)).toBe(text);
  });
