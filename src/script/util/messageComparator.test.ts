...
describe('MessageComparator', () => {
  it('areMentionsDifferent', () => {
    ...
    tests.forEach(({expected, newMentions, originalMentions}) => {
      ...
      expect(areMentionsDifferent(messageEntity as ContentMessage, newMentions as MentionEntity[])).toBe(expected);
    });
  });
  ...
  it('isTextDifferent', () => {
    ...
    tests.forEach(({expected, newText, originalText}) => {
      ...
      expect(isTextDifferent(messageEntity as ContentMessage, newText)).toBe(expected);
    });
  });
});
