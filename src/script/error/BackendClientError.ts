...
import {BaseError, BaseErrorType} from './BaseError';

export enum BackendClientErrorType {
  GENERIC = 'GENERIC',
}

export class BackendClientError extends Error {
  static readonly TYPE = BackendClientErrorType;
  ...
  constructor(type: BackendClientErrorType, message?: string) {
    super();

    this.name = this.constructor.name;
    this.stack = new Error().stack;

    this.type = BackendClientErrorType[type] || BaseErrorType.UNKNOWN;
    this.message = message || `${BaseError.MESSAGE.UNKNOWN} ${name}`;
  }
}
