...
ko.components.register('user-actions', {
  ...
  viewModel: class {
    readonly items: ko.Computed<MenuItem[]>;
    readonly dispose: () => void;

    constructor(params: ComponentParams) {
      const {onAction = noop, conversation = () => false, actionsViewModel, user} = params;
      const isSelfActivated = ko.unwrap(params.isSelfActivated);
        ...
        },
        {
          ...
          condition: () => {
            return (
              ...
              isSelfActivated &&
              conversation() &&
              (conversation() as Conversation).isGroup() &&
              !(conversation() as Conversation).removed_from_conversation()
            );
          },
          item: {
        ...
        },
        {
          ...
          condition: () => {
            return (
              isNotMe() &&
              conversation() &&
              (conversation() as Conversation).isActiveParticipant() &&
              (conversation() as Conversation).participating_user_ids().some(id => user().id === id) &&
              (z as any).userPermission().canUpdateGroupParticipants()
            );
          },
          item: {
            click: () =>
      ...
      this.items = ko.computed(() =>
        user() ? allItems.filter(({condition}) => condition()).map(({item}) => item) : [],
      );
      this.dispose = () => {
        isMe.dispose();
        isNotMe.dispose();
