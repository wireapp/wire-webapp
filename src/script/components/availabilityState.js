...
ko.components.register('availability-state', {
  template: `
      <!-- ko if: $data.availability() === z.user.AvailabilityType.AVAILABLE -->
        <availability-available-icon class="availability-state-icon" data-uie-name="available"></availability-available-icon>
      <!-- /ko -->
      <!-- ko if: $data.availability() === z.user.AvailabilityType.AWAY -->
        <availability-away-icon class="availability-state-icon" data-uie-name="away"></availability-away-icon>
      <!-- /ko -->
      <!-- ko if: $data.availability() === z.user.AvailabilityType.BUSY -->
        <availability-busy-icon class="availability-state-icon" data-uie-name="busy"></availability-busy-icon>
      <!-- /ko -->
      <!-- ko if: $data.label -->
