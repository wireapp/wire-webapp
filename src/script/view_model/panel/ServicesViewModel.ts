...
export class ServicesViewModel extends BasePanelViewModel {
  ...
  constructor(params: PanelViewModelProps) {
    ...
    this.logger = getLogger('ServicesViewModel');
  ...
  copyLink = async (): Promise<void> => {
    if (!this.isLinkCopied() && this.activeConversation()) {
      ...
      window.setTimeout(() => this.isLinkCopied(false), ServicesViewModel.CONFIG.CONFIRM_DURATION);
    }
  };
