...
import {USER_EVENT, BackendEvent} from '@wireapp/api-client/src/event/';
...
/** An event that is either an event from the core or a fake payload generated by the webapp */
type EventPayload = BackendEvent;
export class EventRepository {
  ...
  async connectWebSocket(account: Account) {
    ...
    account.listen({
      ...
      onEvent: (payload, source) => {
        this.handleEvent(payload, source);
      },
    });
  }
  ...
  */
   ...
   */
  async injectEvent(event: EventPayload, source: EventSource = EventSource.INJECTED): Promise<EventPayload> {
    if (!event) {
   ...
   */
  private handleEvent(
    {event, decryptedData}: {event: BackendEvent; decryptedData?: GenericMessage},
    source: EventSource,
  ): Promise<EventRecord> {
   ...
   */
  private handleEventDistribution(event: EventRecord, source: EventSource): Promise<EventPayload> {
   ...
   */
  private handleEventSaving(event: EventPayload): Promise<EventPayload | undefined> {
