...
Beta has one current promotable candidate, even though historical Beta tags remain immutable release history. A candidate becomes current only after its exact artifact has been deployed to Beta, runtime-verified, and given its Beta tag. At that point, the workflow identifies older Release WebApp candidates that have not started an actual Production operation as superseded. Older workflows and pending approvals may remain visible in GitHub; a superseded candidate cannot modify Production because the Production operation verifies the live Beta identity immediately before mutation. Build failures, Beta deployment failures, and Beta runtime-verification failures do not supersede an older candidate.
...
After successful Hosted Beta, candidate supersession, and E2E validation, every normal WebApp release advances to the `wire-webapp-prod` GitHub Environment approval gate. This does not deploy automatically: Hosted Production still requires explicit human approval, and any failed release gate prevents the approval step from being reached. Production receives only the exact artifact validated on Beta.
...
- Deployoholics receives separate informational notifications for four release milestones: each verified new Beta candidate after its new Beta tag is created and older candidates have been superseded; a candidate that passes the blocking E2E gate and reaches Production approval readiness; each new Production release after hosted deployment, runtime verification, immutable tagging, and Docker, Helm, and `wire-builds/main` distribution; and any failed release gate.
- The Beta success notification depends on successful hosted Beta deployment/runtime verification, successful creation of the new Beta tag, and successful candidate supersession, so it is sent before E2E, Production approval, or later release stages. Repeated runs that create `beta.2`, `beta.3`, and later tags each produce one Beta notification.
...
- The Production approval job waits for GitHub Environment approval on the production environment. The separate `Deploy to Production` job is the actual Production operation boundary.
- GitHub Environment approval means the workflow pauses before using the production environment until configured reviewers approve or reject the deployment in GitHub. A candidate that is still before the actual Production operation may be superseded while approval is pending; a Production operation that has started is never automatically cancelled.
- Immediately before modifying Production, the deployment job verifies that the live Beta runtime still matches the exact artifact it intends to promote, including version, asset version, source commit, build timestamp, and expected Production-backend REST and WebSocket configuration. If Beta has been replaced, the candidate stops before Production is touched and no rebuild occurs.
...
- A successfully deployed, runtime-verified, and tagged Beta candidate supersedes older Release WebApp candidates that have not started the actual Production operation. Historical Beta tags remain immutable.
- Supersession discovery is scoped to Release WebApp workflow runs in this repository and only identifies older candidates; it never cancels a complete workflow run. Older workflows and pending approvals may remain visible, while the final live-Beta guard prevents a stale candidate from modifying Production. Unexpected or missing GitHub Actions state fails closed and prevents later E2E or Production promotion.
