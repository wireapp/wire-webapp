...
Release-appearance comments are informational and non-gating: only new-format release tags participate, the first new-format Production release establishes the baseline and is a bootstrap release that receives no comments, each Beta candidate processes only its delta, and Production reconstructs the current release's Beta candidate ranges while repairing missing Beta metadata. Existing first-appearance values are immutable, and release tags plus runtime verification remain authoritative. The workflow installs the repository dependencies required by the isolated release-appearance command.
