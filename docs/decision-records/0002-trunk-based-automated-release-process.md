...
- After a verified Beta deployment creates its immutable Beta tag, and after a verified Production deployment creates its immutable Production tag, the workflow adds informational "first appeared in" metadata to the merged pull requests represented by the release commits. Each pull request has one persistent marker comment with Beta and Production values, and each environment value is written only once; subsequent release candidates do not replace an earlier value. Comment processing is non-gating: deployment tags and live runtime verification remain authoritative, and comment failures never move, recreate, or delete release tags or alter deployment state.
