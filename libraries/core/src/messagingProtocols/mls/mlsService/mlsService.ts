...
export class MLSService extends TypedEventEmitter<Events> {
   ...
   */
  get isEnabled() {
    return this._config !== null && this._config !== undefined;
  }
  ...
  private readonly _uploadCommitBundle = async ({
    ...
    try {
      ...
      if (response.failed_to_send !== null && response.failed_to_send !== undefined) {
  ...
  private dispatchNewCrlDistributionPoints(crlNewDistributionPoints: NewCrlDistributionPoints) {
    if (
      ...
      crlNewDistributionPoints !== undefined &&
      crlNewDistributionPoints.length > 0
    ) {
