...
(async () => {
  ...
  const config: CopyConfigOptions | undefined =
    configFile !== null && configFile !== undefined ? configFile.config : undefined;
  ...
  const copyMessage =
    copiedFiles.length !== 0 && !Number.isNaN(copiedFiles.length) ? `Copied ${copiedFiles.length}` : "Didn't copy any";
