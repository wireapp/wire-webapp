...
z.cryptography.CryptographyRepository = class CryptographyRepository {
  ...
  handle_encrypted_event(event) {
    ...
    if (typeof event_data.text === 'string' && event_data.text.length > z.config.MAXIMUM_MESSAGE_LENGTH_RECEIVING) {
      ...
      return Promise.resolve(
        z.conversation.EventBuilder.build_incoming_message_too_big(event, decryption_error, decryption_error.code)
      );
    }
  ...
  _handle_decryption_failure(error, event) {
    ...
    if (is_invalid_message || is_invalid_signature) {
      ...
      this.logger.error(
        `Session '${session_id}' with user '${remote_user_id}' (client '${remote_client_id}') is broken or out of sync. Reset the session and decryption is likely to work again. Error: ${error.message}`,
        error
      );
    } else if (is_remote_identity_changed) {
      // Remote identity changed
      this.logger.error(
        `Remote identity of client '${remote_client_id}' from user '${remote_user_id}' changed: ${error.message}`,
        error
      );
    }
