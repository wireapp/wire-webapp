...
z.entity.Conversation = class Conversation {
  ...
  constructor(conversation_id = '') {
    ...
    this.messages_visible = ko
      .pureComputed(() => {
        ...
        if (
          ...
          !((first_message != null ? first_message.is_member() : undefined) && first_message.is_creation())
        ) {
    ...
    this.has_active_call = ko.pureComputed(() => {
      ...
      return (
        !z.calling.enum.CALL_STATE_GROUP.IS_ENDED.includes(this.call().state()) &&
    ...
    this.display_name = ko.pureComputed(() => {
      ...
      if (this.is_group()) {
        ...
        if (this.participating_user_ets().length > 0) {
          return this.participating_user_ets().map(user_et => user_et.first_name()).join(', ');
        }
