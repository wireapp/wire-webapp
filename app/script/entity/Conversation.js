...
z.entity.Conversation = class Conversation {
    ...
    message_ets = message_ets
      .map((message_et, index) => {
        const previous_message =
          message_ets[index - 1] || this.get_last_message();
  ...
  prepend_messages(message_ets) {
    ...
    message_ets[message_ets.length - 1] = this._check_for_duplicate(
      last_message_et,
    ...
    message_ets = message_ets
      .map((message_et, index) =>
        this._check_for_duplicate(message_et, message_ets[index - 1])
      )
      .filter(message_et => message_et);
  ...
  _check_for_duplicate_nonce(message_et, other_message_et) {
    if (message_et.nonce === other_message_et.nonce) {
      const [
        older_message_et,
        newer_message_et,
      ] = z.entity.Message.sort_by_timestamp([message_et, other_message_et]);

      const message_is_asset_meta =
        message_et.type === z.event.Client.CONVERSATION.ASSET_META;
      const other_message_is_asset_meta =
