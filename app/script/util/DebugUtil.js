...
z.util.DebugUtil = class DebugUtil {
  ...
  block_all_connections() {
    ...
    wire.app.repository.user.users().forEach((user_et) => {
      return block_users.push(this.user_repository.block_user(user_et));
    });
  ...
  break_session(user_id, client_id) {
    ...
    return wire.app.repository.cryptography.cryptobox.session_load(session_id)
      .then(function(cryptobox_session) {
        ...
        return wire.app.repository.storage.storage_service.save(z.storage.StorageService.prototype.OBJECT_STORE_SESSIONS, session_id, record);
      })
      .then(() => {
