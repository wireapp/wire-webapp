...
z.ViewModel.ConversationInputViewModel = class ConversationInputViewModel {
  constructor(
    ...
    this.pasted_file.subscribe(blob => {
      if (blob) {
        ...
        const date = moment(blob.lastModifiedDate).format(
          'MMMM Do YYYY, h:mm:ss a',
        );
        this.pasted_file_name(
          z.l10n.text(z.string.conversation_send_pasted_file, date),
        );
      } else {
    ...
    this.conversation_input_emoji = new z.ViewModel
      .ConversationInputEmojiViewModel(properties_repository);
  ...
  _show_upload_warning(image) {
    const string_id = image.type === 'image/gif'
      ? z.string.alert_gif_too_large
      : z.string.alert_upload_too_large;
    const warning_text = z.l10n.text(
      string_id,
      z.config.MAXIMUM_IMAGE_FILE_SIZE / 1024 / 1024,
    );
