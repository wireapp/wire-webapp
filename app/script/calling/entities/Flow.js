...
z.calling.entities.Flow = class Flow {
  ...
  _create_peer_connection_configuration() {
    return this.calling_repository.get_config().then(({ice_servers}) => {
      return {
  ...
  send_message(call_message_et) {
    ...
    if (this.data_channel && this.data_channel_opened) {
      try {
        ...
        this.logger.info(
          `Sending '${type}' message to conversation '${conversation_id}' via data channel`,
  ...
  _set_remote_sdp() {
    ...
    this.peer_connection
      .setRemoteDescription(remote_sdp)
      .then(() => {
