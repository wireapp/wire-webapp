...
class z.conversation.ConversationService
  ...
  load_events_from_db: (conversation_id, offset, limit = z.config.MESSAGES_FETCH_LIMIT) ->
    return new Promise (resolve, reject) =>
      ...
      .sortBy 'time'
      .then (records) ->
        records = (record for record in records when new Date(record.time).getTime() < offset) if offset
  ...
  load_unread_events_from_db: (conversation_et, offset) ->
    return new Promise (resolve, reject) =>
      ...
      .then (records) ->
        records = (record for record in records when new Date(record.time).getTime() < offset) if offset
        records = (record for record in records when new Date(record.time).getTime() > conversation_et.last_read_timestamp())
        resolve records
      .catch (error) =>
        @logger.log @logger.levels.ERROR,
