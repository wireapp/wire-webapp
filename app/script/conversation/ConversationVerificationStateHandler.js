...
z.conversation.ConversationVerificationStateHandler = class ConversationVerificationStateHandler {
    ...
    this._get_active_conversations().forEach((conversation_et) => {
      if (this._will_change_to_degraded(conversation_et)) {
        ...
        if (user_ids_in_conversation.length) {
          return amplify.publish(z.event.WebApp.EVENT.INJECT, z.conversation.EventBuilder.build_degraded(conversation_et, user_ids, z.message.VerificationMessageType.NEW_DEVICE));
        }
      }
    });
  }
  ...
  on_member_joined(conversation_et, user_ids) {
    ...
    if (this._will_change_to_degraded(conversation_et)) {
      amplify.publish(z.event.WebApp.EVENT.INJECT, z.conversation.EventBuilder.build_degraded(conversation_et, user_ids, z.message.VerificationMessageType.NEW_MEMBER));
    }
  }
