...
export class QualityController {
  private readonly logger = getLogger('QualityController');
  ...
  constructor(targetFps: number, maxTier: QualityTier | null = null) {
    ...
    if (resolvedTargetFps !== targetFps) {
      this.logger.warn('invalid targetFps, falling back', {
        targetFps,
        resolvedTargetFps,
      });
    }
  ...
  public getTier(mode: Mode): QualityTierParams {
    ...
    return TIER_DEFINITIONS[this.tier];
  }
   ...
   */
  private downgradeTier(): QualityTier {
    if (this.tier === QUALITY_TIERS.FHD) {
      return QUALITY_TIERS.HD;
    }

    if (this.tier === QUALITY_TIERS.HD) {
      return QUALITY_TIERS.QHD;
    }

    if (this.tier === QUALITY_TIERS.QHD) {
      return QUALITY_TIERS.NHD;
    }
    if (this.tier === QUALITY_TIERS.NHD) {
      return QUALITY_TIERS.NHD;
    }
  ...
  private upgradeTier(): QualityTier {
    if (this.tier === QUALITY_TIERS.BYPASS) {
      return this.canUpgradeTo(QUALITY_TIERS.NHD) ? QUALITY_TIERS.NHD : QUALITY_TIERS.BYPASS;
    }

    if (this.tier === QUALITY_TIERS.NHD) {
      return this.canUpgradeTo(QUALITY_TIERS.QHD) ? QUALITY_TIERS.QHD : QUALITY_TIERS.NHD;
    }

    if (this.tier === QUALITY_TIERS.QHD) {
      return this.canUpgradeTo(QUALITY_TIERS.HD) ? QUALITY_TIERS.HD : QUALITY_TIERS.QHD;
    }

    if (this.tier === QUALITY_TIERS.HD) {
      return this.canUpgradeTo(QUALITY_TIERS.FHD) ? QUALITY_TIERS.FHD : QUALITY_TIERS.HD;
    }
