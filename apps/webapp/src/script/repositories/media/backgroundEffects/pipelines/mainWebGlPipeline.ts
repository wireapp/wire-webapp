...
export class MainWebGlPipeline implements BackgroundEffectsRenderingPipeline {
    ...
    if (!qualityTier.bypass && qualityTier.segmentationCadence > 0 && this.segmenter && this.qualityController) {
      if (frameIndex % qualityTier.segmentationCadence === 0) {
        ...
        if (timestampMs <= this.lastSegmentationTimestampMs) {
          this.logger.warn('Dropping frame due to non-monotonic timestamp', {
            timestampMs,
        ...
        this.logger.debug('Segmenting timestampMs: ', timestampMs);
  ...
  private getMonotonicTimestampMs(inputTimestamp: number): number {
    // Wenn inputTimestamp in Sekunden kommt:
