...
describe('AudioSeekBarNew', () => {
  ...
  it('renders level indicators for the audio asset', () => {
    const {container} = render(<AudioSeekBarNew {...getDefaultProps()} />);
    expect(container.querySelectorAll('path')).toHaveLength(2);
  });

  it('updates on audio events', () => {
    const audioElement = createAudioElement(500);
    const {container} = render(<AudioSeekBarNew {...getDefaultProps()} audioElement={audioElement} />);
  ...
  it('updates the currentTime on click', () => {
    ...
    const {container} = render(<AudioSeekBarNew {...props} />);
