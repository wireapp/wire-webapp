...
export const Message = (props: MessageParams & {scrollTo?: ScrollToElement}) => {
  ...
  useLayoutEffect(() => {
    if (messageElementRef.current === null || messageElementRef.current === undefined) {
      return;
    }
    if (isHighlighted) {
  ...
  useEffect(() => {
    if (messageElementRef.current === null || messageElementRef.current === undefined || message.isContent()) {
      return;
    }
  ...
  return (
    ...
    >
      {onVisible !== null && onVisible !== undefined ? (
        <InViewport
          requireFullyInView
