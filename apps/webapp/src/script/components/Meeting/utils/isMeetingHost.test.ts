...
describe('isMeetingHost', () => {
  it('returns true when the self user is the meeting creator', () => {
    expect(isMeetingHost(createMeeting(), createSelfUser())).toBe(true);
  });

  it('returns false when the self user is not the meeting creator', () => {
    expect(isMeetingHost(createMeeting(), createSelfUser('other-user'))).toBe(false);
  });
});
