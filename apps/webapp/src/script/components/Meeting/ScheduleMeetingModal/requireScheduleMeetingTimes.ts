...
export const requireScheduleMeetingTimes = (formState: ScheduleMeetingFormState): {start: Date; end: Date} => {
  if (formState.start.isNothing || formState.end.isNothing) {
    throw new Error('Schedule meeting form is missing start or end time');
  }

  return {start: formState.start.value, end: formState.end.value};
};
