...
test.describe('Thread', () => {
  ...
  test('I can start a thread from a message and keep focus after send', async ({createPage}) => {
    ...
    await expect(pages.conversation().threadBreadcrumbThread).toBeVisible();
  ...
  test('I can open thread from the replies badge under the root message', async ({createPage}) => {
    ...
    await pages.conversation().closeMainPaneThreadViaBreadcrumb();
  ...
  test('I can open the conversation threads list from the header button', async ({createPage}) => {
    const pages = (await PageManager.from(createPage(withLogin(userA), withConnectedUser(userB)))).webapp.pages;

    await pages.conversation().sendMessage('Threads list root');
    const rootMessage = pages.conversation().getMessage({content: 'Threads list root'});

    await pages.conversation().startThreadForMessage(rootMessage);
    await pages.conversation().sendThreadMessage('Threads list reply');
    await pages.conversation().closeMainPaneThreadViaBreadcrumb();

    await pages.conversation().openConversationThreadsList();
    await expect(pages.conversation().conversationThreadsListPanel).toBeVisible();
    await expect(pages.conversation().conversationThreadsListPanel.getByTestId('conversation-threads-list-item')).toHaveCount(1);
  });
});
