...
test.describe('account settings', () => {
  ...
  test(
    ...
    async ({createPage, api}, testInfo) => {
      ...
      for (const {firstName, expectedUsername} of usernameScenarios) {
        await test.step(`Scenario: ${firstName} -> ${expectedUsername}`, async () => {
          ...
          await test.step('Verify autogenerated username on registration and profile', async () => {
            const defaultUserName = await pages.setUsername().getHandleInputValue();
            expect(defaultUserName).toContain(expectedUsername);
