...
interface SlateLink { t: "a"; url: string; children: SlateText[] }
...
function extractTcTags(title: string, tags: string[]): string[] {
    ...
    for (const t of [title, ...tags]) {
        String(t).match(/@?TC-\d+/gi)?.forEach(m => matches.add(m.replace(/^@/, "").toUpperCase()));
    }
    return [...matches];
}
...
async function resolveTestCases(allTests: FlatTest[], runId: number) {
    ...
    for (const test of allTests) {
        ...
        if (tcKeys.length === 0) { console.warn(`  ⚠️  SKIP (no @TC tag): ${test.fullTitle}`); skippedNoTag++; continue; }
